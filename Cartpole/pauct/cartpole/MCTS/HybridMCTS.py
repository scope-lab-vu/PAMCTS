...
class HybridMCTS():
    ...
    def update_tree(self, action):
        ...
        # TODO | probably fine
        self.root = MCTSNode(None, None, c_puct=self.c_puct, policy_value=None, alpha=self.alpha)
    ...
    def compute_plan(self, env, starting_state):
        ...
        for iteration in range(self.num_iterations):
            ...
            if not done and depth < self.max_depth: # it's a leaf node. let's add its children
                ...
                node.children = [MCTSNode(node, a, c_puct=self.c_puct, policy_value=policy_values[a], alpha=self.alpha) for a in self.combinations(env_mcts.action_space)]
