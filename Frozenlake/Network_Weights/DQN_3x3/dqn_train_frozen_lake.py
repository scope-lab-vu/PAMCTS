...
env = Customized_FrozenLakeEnv(desc=custom_map, is_slippery=True, customized_prob1=1.0,
                               customized_prob2=0.0, customized_prob3=0.0)
...
dqn.save_weights(f'duel_dqn_{ENV_NAME}_slip_weights_reorder.h5f', overwrite=True)
...
print('retrain computation time: {}'.format(end_time - start_time))
print('done')
# write computation time to file
with open('retrain_computation_time.txt', 'a') as f:
    f.write('retrain computation time: {}'.format(end_time - start_time))
