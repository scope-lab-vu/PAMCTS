from MCTS.frozenlake.stochasticMCTSNode import ActionNode, StateNode
import copy
...
class StochasticMCTS:
    ...
    def compute_plan(self, env, starting_state):
        ...
        self.root = self.create_state_node(action_node=None,
                                           new_state=copy.deepcopy(search_env.env.s),
                                           depth=0,
        ...
        for iteration in range(self.num_iterations):
            ...
            search_env.reset()
            search_env.env.s = self.root.state
    ...
    def get_action(self, observation, env):
        return self.compute_plan(env=env, starting_state=observation)

    def get_action_scores(self, observation, env):
        action = self.compute_plan(env=env, starting_state=observation)
        action_scores = {}
