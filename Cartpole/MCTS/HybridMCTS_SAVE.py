...
from MCTS.MCTSNodeSAVE import MCTSNodeSAVE
import copy
from gym import spaces
import itertools
from envs.discrete_w_start import DiscreteWS

class HybridMCTSSAVE():
    ...
    def __init__(self,
        ...
        self.gamma = gamma
        # self.alpha = alpha
        # self.root = MCTSNodeSAVE(None,
        #                      None,
        #                      policy_value=self.learning_agent.get_q_values(_rollout_state).max(),
        #                      alpha=self.alpha)
    ...
    def compute_plan(self, env, starting_state):
        ...
        self.root = MCTSNodeSAVE(None,
                                 None,
                                 policy_value=self.learning_agent.get_q_values(starting_state).max(),
                                 # alpha=self.alpha
                                 )
        ...
        for iteration in range(self.num_iterations):
            ...
            curr_state = copy.deepcopy(starting_state) # TODO | this should be the starting state, yes?

            while node.children:
                node = max(node.children, key=lambda node: node.SAVE_score)
            ...
            if not done and depth < self.max_depth: # it's a leaf node. let's add its children
                ...
                # node.children = [MCTSNodeSAVE(node, a, policy_value=policy_values[a], alpha=self.alpha) for a in self.combinations(env_mcts.action_space)]
                node.children = [MCTSNodeSAVE(node, a, policy_value=policy_values[a]) for a in self.combinations(env_mcts.action_space)]
            ...
            # rollout with a random policy till we reach a terminal state
                ...
                while not done and depth < self.max_depth and leaf_rollout_depth < self.rollout_depth:
                    _rollout_state, reward, done, _ = env_mcts.step(env_mcts.action_space.sample()) # TODO greedy rollout for k-server?

                    # TODO | shouldn't gamma's exponent be the total depth, not just leaf_rollout_depth? - actually, I think it's fine since gamma is added again during backprop
                    leaf_rollout_return += self.gamma ** leaf_rollout_depth * reward  # discounted
                    leaf_rollout_depth += 1
                    depth+=1

                if not done and leaf_rollout_depth >= self.rollout_depth:
                    # leaf_rollout_depth += 1
                    if _rollout_state is None:
                        max_depth_q_value = self.learning_agent.get_q_values(curr_state).max()
                    else:
                        max_depth_q_value = self.learning_agent.get_q_values(_rollout_state).max()

                    leaf_rollout_return += self.gamma ** leaf_rollout_depth * max_depth_q_value  # discounted
