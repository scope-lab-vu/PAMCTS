from .MCTSNode_AZ import MCTSNode_AZ
import copy
...
class HybridMCTS_AZ():
    ...
    def __init__(self,
        ...
        self.alpha = alpha
        self.c_puct = c_puct
        self.root = MCTSNode_AZ(None, None, c_puct=self.c_puct, policy_value=None, alpha=self.alpha, state=None)
    ...
    def update_tree(self, action, starting_state):
        """
        ...
        # TODO | probably fine
        self.root = MCTSNode_AZ(None, None, c_puct=self.c_puct, policy_value=None, alpha=self.alpha, state=starting_state)
    ...
    def evaluate_mcts_policy(root_node, env, max_depth=500, render=True):
        ...
        while root_node and root_node.best_child and not done and depth < max_depth:
            ...
            next_state, r, done, info = env_backup.step(best_action)
            # _, r, done, info = env_backup.step(best_action)
            cum_reward += r
            depth += 1
            root_node = root_node.best_child
            root_node.update_state(next_state)
            if render:
                env_backup.render()
    ...
    def compute_plan(self, env, starting_state):
        ...
        for iteration in range(self.num_iterations):
            ...
            if not done and depth < self.max_depth: # it's a leaf node. let's add its children
                ...
                # node.children = [MCTSNode_AZ(node, a, policy_value=policy_values[a], alpha=self.alpha) for a in self.combinations(env_mcts.action_space)]
                for a in self.combinations(env_mcts.action_space):
                    temp_env = copy.deepcopy(env_mcts)
                    expanded_state, reward, _, _ = temp_env.step(a)
                    policy_value = self.learning_agent.get_q_value(expanded_state)
                    node.children.append(MCTSNode_AZ(node, a, c_puct=self.c_puct, policy_value=policy_value, alpha=self.alpha, state=expanded_state))
                ...
                while not done and depth < self.max_depth:
                    # _rollout_state, reward, done, _, _ = env_mcts.step(env_mcts.action_space.sample())
    ...
    def get_action(self, observation, env):
        self.update_tree(None, observation)
