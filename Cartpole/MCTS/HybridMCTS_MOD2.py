...
class HybridMCTSMod2():

    '''
    difference is that this one returns learned q value at max depth in rollout
    '''
        ...
        for iteration in range(self.num_iterations):
            ...
            while node.children:
                ...
                if depth >= self.max_depth:
                    ...
                    trajectory_rewards[depth - 1] += max_depth_q_value
                    # depth += 1
                    break
                if done:
                    break
            ...
            if not done and depth < self.max_depth: # it's a leaf node. let's add its children
                ...
                while not done and depth < self.max_depth:
                    _rollout_state, reward, done, _ = env_mcts.step(env_mcts.action_space.sample())
                ...
                # TODO | I think this is wrong: stopping condition should either the max-depth OR some rollout depth (can be zero)
                if not done and depth == self.max_depth:
                    if _rollout_state is None:
                        # max_depth_q_value = random.sample(self.learning_agent.get_q_values(curr_state), 1)[0] # TODO make this max
                        max_depth_q_value = self.learning_agent.get_q_values(curr_state).max()
                    else:
                        # max_depth_q_value = random.sample(self.learning_agent.get_q_values(_rollout_state), 1)[0]
                        max_depth_q_value = self.learning_agent.get_q_values(_rollout_state).max()
                    # _rollout_state, reward, done, _ = env_mcts.step(env_mcts.action_space.sample())
                    # value = self.learning_agent.get_q_values(_rollout_state)
