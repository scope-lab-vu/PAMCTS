...
def evaluate_policy(policy, verbose, network, iterations, c_puct, prob_distribution, sample_id, gamma, alpha, file_name):
    ...
    results = [[outcomes[0], prob_distribution, iterations, sample_id, time.time() - start_time, lengths[0], c_puct, alpha]]
    if not os.path.exists(file_name):
        with open(file_name, "w") as f:
            pd.DataFrame(results).to_csv(f, header=["cumulative_reward", "prob_distribution", "iterations",
                                                    "sample id", "computation time", "step_counter", "c_puct", "alpha"],
                                         index=False)
    else:
...
if __name__ == "__main__":
    num_cpus = 30
    saved_file_name = "frozenlake_alpha_selection_results_final_part2.csv"
    ...
    #                   0.80, 0.85, 0.90, 0.95, 1.0]:
        for alpha in [1.0]:
            # for iterations in [5, 10, 15, 25, 50, 100, 200, 500]:
            for iterations in [5]:
                for c_puct in [1.414]:
                    for gamma in [0.95]:
    ...
    with open("frozenlake_alpha_selection_execution_time_final_part2.txt", "w") as f:
        f.write(f"fl final alpha selection experiments execution time: {time.time() - begin_time}")
